...
2. **Extrae el archivo:**
   ...
   Descomprime el archivo .zip que has descargado en una ubicación de tu elección, por ejemplo, C:\Herramientas\Git.

3. **Añade Git al PATH del sistema:**
   
   Haz clic derecho en "**Este PC**" o "**Mi PC**" en el Explorador de Archivos y selecciona Propiedades. Luego, haz clic en **Configuración avanzada del sistema** y después en **Variables de entorno**. Busca la variable Path en "**Variables del sistema**" y haz clic en **Editar**. Añade la ruta donde se encuentra el ejecutable de Git (por ejemplo, C:\Herramientas\Git\cmd) y haz clic en Aceptar para guardar los cambios.
...
- Aumenta la capacidad de Git para clonar archivos grandes:
